...
function addStats(target: FighterStats, source: FighterStats) {
  for (const key of Object.keys(target) as Array<keyof FighterStats>) {
...
function divideStats(stats: FighterStats, divisor: number): FighterStats {
  ...
  for (const key of Object.keys(result) as Array<keyof FighterStats>) {
    result[key] = stats[key] / Math.max(1, divisor);
  }
  return result;
}
