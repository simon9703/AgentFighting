...
export default function ThreeArenaViewport({ view, selectedFighterId, quality = 'high' }: Props) {
  ...
  useEffect(() => {
    ...
    scene.add(key);
    const rim = new THREE.PointLight(0x744cff, 78, 38, 2);
    rim.position.set(-2, 5.2, -7);
    scene.add(rim);
    const warm = new THREE.PointLight(0xff8a4d, 28, 24, 2);
    warm.position.set(8, 2.5, 4);
    scene.add(warm);

    const world = new THREE.Group();
    scene.add(world);
    const floor = new THREE.Mesh(
      new THREE.CylinderGeometry(10.4, 10.8, 1.05, 64),
      new THREE.MeshStandardMaterial({ color: 0x0f1620, roughness: 0.78, metalness: 0.42 }),
    );
    ...
    const lowerDeck = new THREE.Mesh(
      new THREE.CylinderGeometry(11.7, 12.3, 0.55, 64),
      new THREE.MeshStandardMaterial({ color: 0x090d14, roughness: 0.65, metalness: 0.7 }),
    );
    lowerDeck.position.y = -1.12;
    world.add(lowerDeck);

    const ringMaterial = new THREE.MeshBasicMaterial({ color: 0x56e1ff, transparent: true, opacity: 0.52, side: THREE.DoubleSide });
    const ring = new THREE.Mesh(new THREE.RingGeometry(8.95, 9.33, 72), ringMaterial);
    ring.rotation.x = -Math.PI / 2;
    ring.position.y = 0.025;
    world.add(ring);
    const outerRing = new THREE.Mesh(
      new THREE.TorusGeometry(10.4, 0.09, 8, 96),
      new THREE.MeshStandardMaterial({ color: 0x24354a, emissive: 0x0a5a74, emissiveIntensity: 1.2, metalness: 0.75, roughness: 0.3 }),
    );
    outerRing.rotation.x = Math.PI / 2;
    outerRing.position.y = -0.28;
    world.add(outerRing);

    const grid = new THREE.GridHelper(18, 18, 0x31526a, 0x172536);
    grid.position.y = 0.02;
    world.add(grid);
    const centerPlatform = new THREE.Mesh(
      new THREE.CylinderGeometry(2.35, 2.65, 0.42, 32),
      new THREE.MeshStandardMaterial({ color: 0x172333, roughness: 0.42, metalness: 0.82 }),
    );
    centerPlatform.position.y = 0.12;
    centerPlatform.receiveShadow = true;
    world.add(centerPlatform);
    const centerGlow = new THREE.Mesh(
      new THREE.RingGeometry(1.35, 1.85, 48),
      new THREE.MeshBasicMaterial({ color: 0x4edfff, transparent: true, opacity: 0.22, side: THREE.DoubleSide }),
    );
    ...
    [[-5.0, -2.8, 1.35, 1.65, 3.2], [4.8, 3.0, 1.2, 2.0, 2.8], [-3.8, 4.1, 2.8, 1.05, 1.0], [3.6, -4.1, 2.55, 1.15, 1.0]].forEach(([x, z, sx, sy, sz], index) => {
      ...
      world.add(block);
      const strip = new THREE.Mesh(new THREE.BoxGeometry(sx * 0.74, 0.07, sz * 1.04), accentMaterial);
      strip.position.set(x, sy + 0.035, z);
    ...
    for (let i = 0; i < 18; i += 1) {
      ...
      lamp.rotation.y = wall.rotation.y;
      world.add(lamp);
    }
    ...
    for (let i = 0; i < 6; i += 1) {
      ...
      tower.position.set(Math.cos(angle) * 13.0, 1.6, Math.sin(angle) * 13.0);
      world.add(tower);
      const light = new THREE.PointLight(i % 2 ? 0x6c4dff : 0x3fdcff, 18, 12, 2);
      light.position.set(tower.position.x, 4.2, tower.position.z);
      scene.add(light);
    }
    ...
    const syncFighters = () => {
      ...
      current.fighters.forEach((fighter) => {
        ...
        if (!visual) {
          const robot = createFighterRig(fighter.color);
        ...
        (visual.intentRing.material as THREE.MeshBasicMaterial).opacity = selectedRef.current === fighter.id
          ? 0.94
          : fighter.intent.toLowerCase().includes('attack')
            ? 0.84
            : 0.34;
        ...
        if (fighter.intent !== visual.lastIntent) {
          ...
          if (intent.includes('heavy')) {
            triggerFighterRig(visual, 'heavy', 1);
            fx.slash(position, facing, fighter.color, true);
          } else if (intent.includes('attack')) {
            triggerFighterRig(visual, 'attack', 1);
            fx.slash(position, facing, fighter.color, false);
          }
          if (intent.includes('dodge') && visual.velocity.lengthSq() > 0.01) {
            triggerFighterRig(visual, 'dodge', 1);
            fx.dodge(position, visual.velocity, fighter.color);
          }
    ...
    const syncWeapons = () => {
      ...
      current.weapons.forEach((weapon) => {
        ...
        visual.root.position.copy(worldPosition(weapon.x, weapon.z, current.radius, 0.82));
      });
    ...
    const loop = () => {
      ...
      fighterVisuals.forEach((visual, id) => {
        ...
        if (fighter) {
          updateFighterRig(visual, {
            dt,
            time: now,
            speed: visual.velocity.length(),
            intent: fighter.intent,
            selected: selectedRef.current === id,
            respawning: fighter.respawning,
            eliminated: fighter.eliminated,
            reducedMotion,
          });
        }
        const moved = visual.root.position.distanceTo(visual.lastPosition);
        if (!reducedMotion && moved > 0.08 && visual.velocity.length() > 2.4 && Math.random() < (quality === 'high' ? 0.34 : 0.12)) {
      ...
      weaponVisuals.forEach((visual) => {
        ...
        visual.root.position.y = 0.78 + Math.sin(performance.now() * 0.003 + visual.root.position.x) * 0.12;
      ...
      if (newest && newest.id !== lastEventId) {
        ...
        const actorVisual = actor ? fighterVisuals.get(actor.id) : undefined;
        const targetVisual = target ? fighterVisuals.get(target.id) : undefined;
        const subject = target ?? actor;
        const color = eventColor(newest.type);

        if (newest.type === 'hit' && targetVisual) triggerFighterRig(targetVisual, 'hit', 1);
        if (newest.type === 'stock-lost' && targetVisual) triggerFighterRig(targetVisual, 'ko', 0.78);
        if (newest.type === 'eliminated' && targetVisual) triggerFighterRig(targetVisual, 'ko', 1);
        if (newest.type === 'weapon-use' && actorVisual) triggerFighterRig(actorVisual, 'attack', 0.72);
        ...
        if (subject) {
          ...
          if (['hit', 'weapon-use', 'stock-lost', 'eliminated', 'weapon-pickup', 'chaos'].includes(newest.type)) {
            ...
            fx.burst({
              position: position.clone().setY(newest.type === 'eliminated' ? 1.2 : 0.72),
              color,
              count: newest.type === 'eliminated' ? 54 : newest.type === 'stock-lost' ? 34 : 20,
              speed: newest.type === 'eliminated' ? 7.5 : 4.8,
              life: newest.type === 'eliminated' ? 0.9 : 0.52,
              size: newest.type === 'eliminated' ? 1.35 : 1,
            });
          }
        }
        if (actor && target) {
        ...
        if (newest.type === 'hit') {
          cameraDirector.impact(0.2, 'combat');
          post.hit(0.24);
        } else if (newest.type === 'stock-lost') {
          cameraDirector.impact(0.42, 'ko');
          post.hit(0.52);
        } else if (newest.type === 'eliminated') {
          cameraDirector.impact(0.58, 'ko');
          post.hit(0.76);
        } else if (newest.type === 'weapon-use') {
          cameraDirector.impact(0.14, 'combat');
          post.hit(newest.detail.toLowerCase().includes('bomb') ? 0.48 : 0.18);
        }
      }
      ...
      rim.color.setHex(danger ? 0xc85cff : 0x744cff);
      rim.intensity = danger ? 105 : 78;
      renderer.toneMappingExposure = danger ? 1.25 : 1.2;
      post.render();
    };
    loop();
