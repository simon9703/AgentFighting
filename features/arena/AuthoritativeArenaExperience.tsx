    ...
    <main className={styles.shell}>
      ...
      <header className={styles.topbar}>
        ...
        <div className={styles.liveState}>
          <i /><span>{view.winnerId ? 'MATCH COMPLETE' : playing ? 'LIVE SIMULATION' : 'PAUSED'}</span><b>SEED 9703</b>
        </div>
      ...
      <section className={styles.matchHeader}>
        <div><div className={styles.kicker}>ROUND 01 / AUTHORITATIVE ARENA</div><h1>Locked strategies. Shared world. <span>Only behavior wins.</span></h1></div>
        <div className={styles.timerBlock}><small>TIME REMAINING</small><strong>{Math.max(0, view.timeLeft).toFixed(1)}</strong><span>SECONDS</span></div>
      </section>

      <section className={styles.combatLayout}>
        <aside className={styles.leftRail}>
          <div className={styles.railLabel}>CONTENDERS · TAP TO FOLLOW</div>
          {view.fighters.map((fighter, index) => (
            <article
              key={fighter.id}
              className={`${styles.fighterCard} ${fighter.eliminated ? styles.eliminated : ''}`}
              role="button"
              tabIndex={0}
              onClick={() => setSelectedFighterId((current) => current === fighter.id ? undefined : fighter.id)}
              onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedFighterId((current) => current === fighter.id ? undefined : fighter.id); }}
              style={{ cursor: 'pointer', borderColor: selectedFighterId === fighter.id ? fighter.color : undefined, boxShadow: selectedFighterId === fighter.id ? `0 0 28px ${fighter.color}22` : undefined }}
            >
              <div className={styles.fighterCardTop}>
                ...
                <div className={styles.fighterIdentity}><strong>{fighter.name}</strong><small>{fighter.id}</small></div>
              </div>
              <div className={styles.damageRow}>
                <strong>{Math.round(fighter.damage)}<small>%</small></strong>
                <div className={styles.stockPips}>{Array.from({ length: 3 }).map((_, stockIndex) => <i key={stockIndex} className={stockIndex < fighter.stocks ? styles.stockAlive : ''} />)}</div>
              </div>
              <div className={styles.intentRow}><span>INTENT</span><b>{fighterStatus(fighter)}</b></div>
              <div className={styles.telemetry}><span>VX {fighter.velocityX.toFixed(1)}</span><span>VZ {fighter.velocityZ.toFixed(1)}</span><span>{fighter.weapon ? fighter.weapon.toUpperCase() : 'UNARMED'}</span></div>
            </article>
          ))}
          ...
          <div className={styles.signalCard}>
            ...
            <div className={styles.signalGrid}>
              <span><small>TICK</small><b>{view.tick}</b></span><span><small>RADIUS</small><b>{view.radius.toFixed(1)}m</b></span>
              <span><small>CHAOS</small><b>{view.chaos.toUpperCase()}</b></span><span><small>CAMERA</small><b>{focused?.name ?? 'AUTO'}</b></span>
            </div>
          </div>
        </aside>
        ...
        <section className={styles.stageShell}>
          ...
          <div className={styles.viewport}>
            ...
            {latestEvent && <div className={styles.actionBanner} key={latestEvent.id}><small>LIVE EVENT #{latestEvent.id}</small><strong>{formatEvent(latestEvent)}</strong></div>}
        ...
        <aside className={styles.rightRail}>
          <div className={styles.eventPanel}>
            <div className={styles.panelHeading}><div><div className={styles.railLabel}>EVENT STREAM</div><strong>Combat telemetry</strong></div><span>{view.events.length}</span></div>
            <div className={styles.events}>
              {view.events.slice(-MAX_EVENTS).reverse().map((event, index) => (
                <div key={event.id} className={`${styles.event} ${index === 0 ? styles.latest : ''}`}>
                  <div className={styles.eventLine} /><time>#{String(event.id).padStart(3, '0')}</time>
                  <div><b>{event.type.replaceAll('-', ' ').toUpperCase()}</b><span>{event.actor ?? 'arena'}{event.target ? ` → ${event.target}` : ''}</span><small>{event.detail}</small></div>
                </div>
              ))}
      ...
      <footer className={styles.footer}><span>AGENT FIGHTING / BEHAVIOR-FIRST EVALUATION</span><span>SEEDED · DETERMINISTIC · REPLAYABLE</span><span>ENGINE 30HZ / VISUAL 60FPS</span></footer>
    </main>
  );
}
